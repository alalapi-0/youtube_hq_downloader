...
OpenRouter 用于 AI 搜索计划、语义复筛和反馈分析。YouTube API 是可选项；未配置时，系统会提示搜索能力受限，但不会崩溃。
...
- 没有 `YOUTUBE_API_KEY`：自动关键词搜索能力受限；可先配置 Key，或用高级 CLI 分析已有 URL 列表。
